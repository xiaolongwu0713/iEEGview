...
if exist(getenv("FREESURFER_HOME"), 'dir');
...
yesMRI = exist(strcat(subjectDirectory, '/DICOM/',MRI_folder_name,'/'), 'dir');
...
yesCT = exist(strcat(subjectDirectory, '/DICOM/',CT_folder_name), 'dir');
...
mriDICOMpath = strcat(subjectDirectory, '/DICOM/',MRI_folder_name,'/');
ctDICOMpath = strcat(subjectDirectory, '/DICOM/',CT_folder_name,'/');
...
if segment
    ...
    subjectExists = exist(strcat(getenv("FREESURFER_HOME"), '/subjects/', subjectID), 'dir');
    ...
    if subjectExists
        ...
        tmptext{end+1} = ['It appears that there is already a subject folder in freesurfer for this subject.' ...
            'Either delete this folder (' getenv("FREESURFER_HOME") '/subjects/' subjectID '/) or change the subjectID.'];
        fprintf(handles.ProcessState,[tmptext{end} '\r\n']);
    ...
    for i = 1:3
        ...
        notRunning = ~exist(strcat(getenv("FREESURFER_HOME"), '/subjects/', subjectID, '/scripts/IsRunning.lh+rh'), 'file');
        already_done=true;
        if i == 3 && notRunning
            fprintf('The file %s/subjects/%s/scripts/IsRunning.lh+rh\n', getenv("FREESURFER_HOME"), subjectID);
            ...
            tmptext = get(handles.edit_State,'string');
            tmptext{end+1} = ['The file ',getenv("FREESURFER_HOME"),'/subjects/',subjectID,'/scripts/IsRunning.lh+rh'...
                ...
                'in ',getenv("FREESURFER_HOME"),'/subjects/',subjectID,'/scripts/recon-all.log\n']
            fprintf(handles.ProcessState,[tmptext{end} '\r\n']);
            set(handles.edit_State,'string',tmptext(max(length(tmptext)-7,1):end));
        
            return;
        elseif ~notRunning || already_done
            break;
        end
    end
    ...
    fprintf('%s/subjects/%s/scripts/recon-all.log\n\n', getenv("FREESURFER_HOME"), subjectID);
    tmptext = get(handles.edit_State,'string');
    tmptext{end+1} = ['The progress of the segmentation process can be observed by opening ' ...
        getenv("FREESURFER_HOME"),'/subjects/',subjectID,'/scripts/recon-all.log'];
    fprintf(handles.ProcessState,[tmptext{end} '\r\n']);
    set(handles.edit_State,'string',tmptext(max(length(tmptext)-7,1):end));
    ...
    tmptext = get(handles.edit_State,'string');
    tmptext{end+1} = ['When the segmentation is complete the results are saved in ',getenv("FREESURFER_HOME"),'/subjects/',subjectID,'/...' ...
...
if coreg
    ...
    if convert
        ...
        tmp=dir(strcat(subjectDirectory, '/NIfTI/MRI/*.img')).name;
        mrifilename=tmp(1:end-4);
        tmp=dir(strcat(subjectDirectory, '/NIfTI/CT/*.img')).name;
        ctfilename=tmp(1:end-4);
    end
